...
We will rewrite the fee-bump signing engine in Rust using the Axum web framework and Tokio async runtime. The Rust binary (`paymaster-server/`) becomes the primary production backend. The Node.js server (`server/`) is retained as a parity harness and migration aid while the rollout completes, but all new signing work targets the Rust implementation.
...
- **Cons**:
  - Rust has a steeper learning curve than Node.js; onboarding new contributors to `paymaster-server/` takes longer.
