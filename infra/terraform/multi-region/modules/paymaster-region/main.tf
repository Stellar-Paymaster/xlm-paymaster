...
locals {
  name_prefix = "paymaster-${var.environment}-${var.region_name}"
}
...
resource "aws_ecs_task_definition" "api" {
  ...
  container_definitions = jsonencode([
    {
      name  = "api"
      image = "ghcr.io/stellar-paymaster/paymaster-server-ts:${var.image_tag}"
...
resource "aws_ecs_task_definition" "rust_engine" {
  ...
  container_definitions = jsonencode([
    {
      name  = "rust-engine"
      image = "ghcr.io/stellar-paymaster/paymaster-server:${var.image_tag}"
...
resource "aws_secretsmanager_secret_version" "db_url" {
  ...
  secret_string = "postgresql://paymaster:${var.db_password}@${local.name_prefix}-db:5432/paymaster_db"
}
