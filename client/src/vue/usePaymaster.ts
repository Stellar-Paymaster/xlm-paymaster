...
export interface UsePaymasterResult {
  ...
  error: Ref<PaymasterError | Error | null>;
  result: ShallowRef<FeeBumpResponse | null>;
}

export function usePaymaster(client: PaymasterClient): UsePaymasterResult {
  const isLoading = ref(false);
  const error = ref<PaymasterError | Error | null>(null);
