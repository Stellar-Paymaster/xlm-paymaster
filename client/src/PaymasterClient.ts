...
export class PaymasterClient {
  ...
  private normalizeServerUrls(config: PaymasterClientConfig): string[] {
    ...
    if (unique.length === 0) {
      throw new PaymasterConfigurationError(
        "PaymasterClient requires at least one server URL via serverUrl or serverUrls"
      );
    }
  ...
  private mapGrpcTransportError(error: unknown, serverUrl: string): Error {
    ...
    if (error.kind === "network") {
      return new PaymasterNetworkError(error.message, serverUrl);
    }

    return new PaymasterServerError(
      error.message,
  ...
  private async performTransportRequest<TResponse>(
    ...
    if (this.transportMode === "grpc-web") {
      if (!encodeGrpcRequest || !decodeGrpcResponse) {
        throw new PaymasterConfigurationError("gRPC transport requires request codecs");
      }
  ...
  private async performJsonRequest<T>(
    ...
    try {
      ...
      throw new PaymasterNetworkError(
        isTimeout 
          ? `Paymaster server request timed out after ${this.timeout}s` 
          : `Paymaster server request failed: ${error instanceof Error ? error.message : String(error)}`,
        serverUrl
      );
    } finally {
    ...
    if (!response.ok) {
      ...
      throw new PaymasterServerError(
        `Paymaster server error: ${response.status} ${response.statusText}`,
        response.status,
        serverUrl,
        parsedError
      );
  ...
  private async requestWithFallback<T>(
    ...
    for (let attemptIndex = 0; attemptIndex < orderedServerUrls.length; attemptIndex += 1) {
      ...
      try {
        ...
        if (error instanceof PaymasterServerError && error.status === 400) {
          throw error;
        }
        ...
        if (attemptIndex < orderedServerUrls.length - 1) {
          ...
          console.warn(
            `[PaymasterClient] Request failed on ${serverUrl} (${lastError.message}). Retrying ${path} on ${nextServerUrl} in ${retryDelayMs}ms.`
          );
    ...
    if (lastError instanceof PaymasterServerError) {
      throw lastError;
    }

    throw new PaymasterNoAvailableServerError(
      "All configured Paymaster servers failed or are in cool-down.",
      "multiple"
    );
  }
  ...
  private initializeWorker(): void {
    try {
      ...
      this.worker.onmessage = (event: MessageEvent<WorkerResponse>) => {
        ...
        if (pending) {
          ...
          if (type === "success") {
            ...
            pending.reject(new PaymasterWalletError(error || "Worker operation failed"));
          }
        }
      };
      ...
      console.warn("[PaymasterClient] Failed to initialize worker, falling back to main thread:", error);
      this.useWorker = false;
    }
  }
  ...
  async submitFeeBumpTransaction(
    ...
    if (!this.horizonServer) {
      throw new PaymasterConfigurationError("Horizon URL not configured");
    }
    ...
    while (Date.now() - startedAt < timeoutMs) {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          throw new PaymasterServerError(`Horizon error while confirming tx: ${res.statusText}`, res.status, this.horizonUrl, body);
        }

        return await res.json();
      } catch (error) {
        if (error instanceof PaymasterServerError || error instanceof PaymasterNetworkError) throw error;
        await this.sleep(pollIntervalMs);
      }
    }
  ...
  async buildAndRequestFeeBump(
    ...
    if (this.useWorker && this.worker) {
      try {
        ...
        console.warn("[PaymasterClient] Worker signing failed, using main thread fallback");
        if (!keypair) throw new PaymasterWalletError("Keypair required for main thread signing fallback");
        signedXdr = await this.signOnMainThread(transaction, keypair);
      }
    } else {
      if (!keypair) throw new PaymasterWalletError("Keypair required for signing");
      signedXdr = await this.signOnMainThread(transaction, keypair);
    }
  ...
  /**
   * Reports a bug or diagnostic information to the Paymaster telemetry service.
