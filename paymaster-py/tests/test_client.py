"""Unit tests for paymaster_py.client.PaymasterClient.
...
TEST_SERVER = "https://paymaster.test"
...
def _make_client(
    ...
    """Return a PaymasterClient configured for testing."""
...
class TestPaymasterClientConfig:
    ...
    def test_accepts_multiple_urls(self) -> None:
        cfg = PaymasterClientConfig(
            server_urls=[TEST_SERVER, "https://paymaster2.test"],
            network_passphrase=TEST_PASSPHRASE,
        )
...
# ---------------------------------------------------------------------------
# PaymasterClient construction
# ---------------------------------------------------------------------------


class TestPaymasterClientInit:
    def test_normalises_trailing_slash(self) -> None:
        client = _make_client(server_url="https://paymaster.test/")
        assert client._server_urls == ["https://paymaster.test"]

    def test_deduplicates_urls(self) -> None:
        client = _make_client(
            server_urls=[TEST_SERVER, TEST_SERVER, "https://paymaster2.test"]
        )
        assert len(client._server_urls) == 2

    def test_repr(self) -> None:
        client = _make_client()
        assert "PaymasterClient" in repr(client)
...
class TestSerialise:
    def test_passthrough_string(self) -> None:
        assert PaymasterClient._serialise(DUMMY_XDR) == DUMMY_XDR

    def test_calls_to_xdr(self) -> None:
        mock_tx = MagicMock()
        mock_tx.to_xdr.return_value = DUMMY_XDR
        result = PaymasterClient._serialise(mock_tx)
    ...
    def test_to_xdr_exception_raises_serialisation_error(self) -> None:
        ...
        with pytest.raises(PaymasterSerializationError, match="Failed to serialise"):
            PaymasterClient._serialise(mock_tx)

    def test_unsupported_type_raises_error(self) -> None:
        with pytest.raises(PaymasterSerializationError, match="Unsupported transaction type"):
            PaymasterClient._serialise(12345)  # type: ignore[arg-type]
...
class TestRequestFeeBump:
    ...
    def test_400_raises_without_fallback(self, requests_mock: Any) -> None:
        ...
        client = _make_client()
        with pytest.raises(PaymasterRequestError) as exc_info:
    ...
    def test_500_marks_failure_and_raises(self, requests_mock: Any) -> None:
        ...
        client = _make_client()
        with pytest.raises(PaymasterNoAvailableServerError):
            client.request_fee_bump(DUMMY_XDR)
...
class TestMultiServerFailover:
    def test_falls_back_to_second_server(self, requests_mock: Any) -> None:
        server1 = "https://paymaster1.test"
        server2 = "https://paymaster2.test"
...
class TestSubmitFeeBumpTransaction:
    def test_raises_without_horizon_url(self) -> None:
        client = _make_client()
        with pytest.raises(PaymasterConfigError, match="horizon_url"):
    ...
    def test_horizon_error_raises_paymaster_request_error(
        self, requests_mock: Any
    ) -> None:
        horizon = "https://horizon-testnet.stellar.org"
        requests_mock.post(
...
class TestNodeFailureState:
    ...
    def test_retry_delay_caps_at_max(self) -> None:
        from paymaster_py.client import _MAX_RETRY_DELAY_S

        large_delay = PaymasterClient._retry_delay(100)
