"""Data models for the Paymaster Python SDK.
...
class PaymasterClientConfig:
    """Configuration for :class:`~paymaster_py.client.PaymasterClient`.
    ...
    Args:
        ...
        server_url: Base URL of a single Paymaster fee-bump server.
        server_urls: Base URLs of multiple Paymaster fee-bump servers.
        ...
        horizon_url: Optional Horizon API URL used by
            :meth:`~paymaster_py.client.PaymasterClient.submit_fee_bump_transaction`.
    ...
    def __post_init__(self) -> None:
        if not self.server_url and not self.server_urls:
            raise ValueError(
                "PaymasterClientConfig requires at least one server URL via "
...
class FeeBumpResponse:
    """Response returned by the Paymaster fee-bump endpoint.
