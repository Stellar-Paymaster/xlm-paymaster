"""Minimal local mock of the Paymaster fee-bump server.

Used for local development and demo purposes when the full Rust/Node
Paymaster server is not available.

Usage::

    python examples/mock_paymaster_server.py          # listens on :3000
    python examples/mock_paymaster_server.py --port 4000
"""
...
log = logging.getLogger("mock-paymaster")
...
class PaymasterMockHandler(BaseHTTPRequestHandler):
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Mock Paymaster fee-bump server")
    parser.add_argument("--port", type=int, default=3000)
    args = parser.parse_args()

    server = HTTPServer(("127.0.0.1", args.port), PaymasterMockHandler)
    log.info("Mock Paymaster server listening on http://127.0.0.1:%d", args.port)
